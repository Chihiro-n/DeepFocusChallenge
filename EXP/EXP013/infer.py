"""
EXP013: LightGBM + DeepResearch features + blur-focused add-ons

- ベース: EXP003（DeepResearch特徴 60個）
- 追加: パワースペクトル重心/半径、LoGピラミッド減衰傾き、弱リブラー差分など

使用方法:
```
python EXP/EXP013/infer.py
```
"""
...
OUTPUT_DIR = Path("EXP/EXP013/outputs")
...
def extract_blur_features(image_path: str) -> dict:
    ...
    # 15. Power spectrum slope + centroid/半径
    radial_energy = []
    radii = []
    for r in range(1, min(crow, ccol)):
        ...
        if mask.sum() > 0:
            ...
            radial_energy.append(band_energy)
            radii.append(r)
    ...
    if len(radial_energy) > 10:
        ...
        log_r = np.log(radii_arr)
        log_power = np.log(radial_energy + 1e-10)
        slope, _ = np.polyfit(log_r, log_power, 1)
        features['spectrum_slope'] = float(slope)

        total_radial = radial_energy.sum()
        centroid = np.sum(radii_arr * radial_energy) / (total_radial + 1e-10)
        features['power_centroid_norm'] = float(centroid / (max_dist + 1e-10))

        cumulative = np.cumsum(radial_energy)
        r50_idx = np.searchsorted(cumulative, total_radial * 0.5)
        r50_val = radii_arr[min(r50_idx, len(radii_arr) - 1)]
        features['power_radius_p50_norm'] = float(r50_val / (max_dist + 1e-10))
    else:
        features['spectrum_slope'] = 0.0
        features['power_centroid_norm'] = 0.0
        features['power_radius_p50_norm'] = 0.0

    # 16. LoGピラミッド減衰傾き（高周波減衰の強さ）
    log_sigmas = np.log(np.array([1.0, 2.0, 3.0, 5.0]))
    log_vars = np.log(np.array([features[f'log_var_s{sigma}'] + 1e-8 for sigma in [1.0, 2.0, 3.0, 5.0]]))
    slope_log, _ = np.polyfit(log_sigmas, log_vars, 1)
    features['log_pyramid_slope'] = float(slope_log)

    # 17. 弱リブラー差分 (σ=1.5)
    reblur_sigma = 1.5
    blurred = cv2.GaussianBlur(img_float, (0, 0), reblur_sigma)
    laplacian_blur = cv2.Laplacian(blurred, cv2.CV_64F)
    sobel_x_blur = cv2.Sobel(blurred, cv2.CV_64F, 1, 0, ksize=3)
    sobel_y_blur = cv2.Sobel(blurred, cv2.CV_64F, 0, 1, ksize=3)
    sobel_mag_blur = np.sqrt(sobel_x_blur ** 2 + sobel_y_blur ** 2)

    lap_var_blur = float(laplacian_blur.var())
    sobel_p95_blur = float(np.percentile(sobel_mag_blur, 95))

    features['reblur_lap_var_ratio_s1p5'] = float(lap_var_blur / (features['laplacian_var'] + 1e-8))
    features['reblur_sobel_p95_drop_s1p5'] = float((features['sobel_p95'] - sobel_p95_blur) / (features['sobel_p95'] + 1e-8))
...
def main():
    print("=" * 60)
    print("EXP013: LightGBM + DeepResearch Features + blur add-ons")
    print("=" * 60)
