"""
EXP004: Train基準Z-score + DeepResearch特徴量 + LightGBM

EXP002とEXP003の融合:
- EXP002: Train画像を基準としたZ-score正規化
- EXP003: DeepResearch特徴量（60個）
- LightGBM

使用方法:
```
python EXP/EXP004/infer.py
```
"""
...
OUTPUT_DIR = Path("EXP/EXP004/outputs")
...
def main():
    print("=" * 60)
    print("EXP004: Train Z-score + DeepResearch + LightGBM")
    print("=" * 60)
    ...
    print(f"\nTrain: {len(train_df)} images (reference)")
    ...
    # 元特徴量カラム
    base_feature_cols = [c for c in train_features.columns if c != 'id']
    print(f"\nBase features: {len(base_feature_cols)}")

    # ============================================
    # Train基準のZ-score計算 (EXP002)
    # ============================================
    print("\n[4/5] Computing Z-score based on train reference...")

    train_mean = train_features[base_feature_cols].mean()
    train_std = train_features[base_feature_cols].std()

    def add_zscore_features(df, base_cols, ref_mean, ref_std):
        """Z-score特徴量を追加"""
        result = df.copy()
        for col in base_cols:
            result[f'{col}_zscore'] = (df[col] - ref_mean[col]) / (ref_std[col] + 1e-8)
        return result

    sample_with_zscore = add_zscore_features(sample_features, base_feature_cols, train_mean, train_std)
    test_with_zscore = add_zscore_features(test_features, base_feature_cols, train_mean, train_std)

    # 全特徴量カラム（元 + Z-score）
    all_feature_cols = base_feature_cols + [f'{c}_zscore' for c in base_feature_cols]
    print(f"Total features (base + zscore): {len(all_feature_cols)}")

    # 学習データ
    X_train = sample_with_zscore[all_feature_cols].values
    y_train = sample_with_zscore['abs_focus'].values
    ...
    for pattern in sorted(sample_with_zscore['pattern'].unique()):
        mask = sample_with_zscore['pattern'].values == pattern
    ...
    X_test = test_with_zscore[all_feature_cols].values
    y_pred_test = final_model.predict(X_test)
    y_pred_test = np.clip(y_pred_test, 0, None)

    # 提出ファイル作成
    submission = pd.DataFrame({
        'id': test_with_zscore['id'],
